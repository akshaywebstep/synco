...
exports.addToWaitingListService = async (data, adminId) => {
  ...
  try {
    ...
    // 2️⃣ Handle "request_to_cancel" or "cancelled" bookings
    if (["request_to_cancel", "cancelled"].includes(booking.status)) {
      // 🔹 Remove entry from cancel booking table
      const existingCancel = await CancelBooking.findOne({
        where: { bookingId: booking.id },
        transaction: t,
      ...
      await booking.update(
        {
          ...
          venueId: data.venueId,
          classScheduleId: data.classScheduleId,
          startDate: null, // ⬅️ reset startDate
          additionalNote: data.additionalNote || booking.additionalNote,
          paymentPlanId: null, // ⬅️ remove payment plan link
          bookedBy: adminId,
        },
        { transaction: t }
      );
      ...
      return {
        status: true,
        message: `Booking moved from ${booking.status} to waiting list successfully.`,
        data: updatedBooking,
      };
    }
    ...
    if (!(booking.bookingType === "paid" && booking.status === "active")) {
      throw new Error(
        `Booking Cannot move to waiting list.`
      );
    }

    // 4️⃣ Validate venue and class schedule
    const venue = await Venue.findByPk(data.venueId, { transaction: t });
    if (!venue) throw new Error("Venue is required.");
