...
exports.createCancellationRecord = async (
  ...
  try {
    ...
    const classSchedule = await HolidayClassSchedule.findByPk(classScheduleId, {
      include: [{ model: HolidayVenue, as: "holidayVenue" }],
    });
    ...
    if (!bookings.length) {
      return {
        status: true,
        message:
          "Cancellation saved & holidayCampDate updated. No bookings, so no emails sent.",
        data: cancelEntry,
      };
    }
    ...
    return {
      status: true,
      message: "Cancellation saved, holidayCampdates updated & emails sent.",
      data: cancelEntry,
    };
  } catch (error) {
