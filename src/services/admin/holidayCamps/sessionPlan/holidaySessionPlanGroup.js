const { HolidaySessionPlanGroup, HolidaySessionExercise } = require("../../../../models");
const { deleteFile } = require("../../../../utils/fileHandler");
const path = require("path");
const { Readable } = require("stream");
const fetch = require("node-fetch");

exports.duplicateHolidaySessionPlanGroup = async (id, createdBy) => {
  try {
    // 1️⃣ Find the original group
    const group = await HolidaySessionPlanGroup.findOne({ where: { id, createdBy } });
    ...
    const newGroup = await HolidaySessionPlanGroup.create(newGroupData);
...
// ✅ Create
exports.createHolidaySessionPlanGroup = async (data) => {
  try {
    const created = await HolidaySessionPlanGroup.create(data);
...
exports.getAllHolidaySessionPlanGroups = async ({
  orderBy = "createdAt",
  order = "DESC",
  createdBy,
} = {}) => {
...
exports.getHolidaySessionPlanGroupById = async (id, createdBy) => {
  try {
...
exports.getHolidaySessionPlanGroupVideoStream = async (id, createdBy, filename) => {
  try {
...
exports.updateHolidaySessionPlanGroup = async (id, updatePayload, createdBy) => {
  try {
    const sessionGroup = await HolidaySessionPlanGroup.findOne({
      where: { id, createdBy },
    });
...
exports.deleteHolidaySessionPlanGroup = async (id, deletedBy) => {
  try {
...
exports.deleteLevelFromHolidaySessionPlanGroup = async (id, levelKey, createdBy) => {
  try {
    const sessionGroup = await HolidaySessionPlanGroup.findOne({
      where: { id, createdBy },
      raw: true,
    ...
    const result = await exports.updateHolidaySessionPlanGroup(
      id,
      updatePayload,
      createdBy
    );
...
exports.reorderHolidaySessionPlanGroups = async (orderedIds = [], createdBy) => {
  try {
    for (let index = 0; index < orderedIds.length; index++) {
      const id = orderedIds[index];
      await HolidaySessionPlanGroup.update(
        { sortOrder: index + 1 },
        { where: { id, createdBy } }
      );
    }

    const updatedGroups = await HolidaySessionPlanGroup.findAll({
      where: { createdBy },
      order: [["sortOrder", "ASC"]],
    ...
    console.error("❌ reorderHolidaySessionPlanGroups service error:", error);
    return { status: false, message: "Internal server error" };
  }
};
