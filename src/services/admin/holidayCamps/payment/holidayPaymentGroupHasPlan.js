const HolidayPaymentGroup = require("../../../../models/admin/holidayCamps/payment/HolidayPaymentGroup");
...
exports.assignPlanToPaymentGroup = async (groupId, planId, createdBy) => {
  try {
    ...
    await HolidayPaymentGroupHasPlan.destroy({
      where: {
    ...
    // Create new mapping
    const created = await HolidayPaymentGroupHasPlan.create({
      payment_group_id: groupId,
      payment_plan_id: planId,
      createdBy,
    });
...
exports.removePlanFromPaymentGroup = async (groupId, planId, createdBy) => {
  try {
    ...
    const deletedCount = await HolidayPaymentGroupHasPlan.destroy({
      where: {
        payment_group_id: groupId,
        payment_plan_id: planId,
        createdBy,
...
exports.getPaymentGroupAssignedPlans = async (groupId, createdBy) => {
  try {
    ...
    const assignedPlans = await HolidayPaymentGroupHasPlan.findAll({
      where: {
        payment_group_id: groupId,
        createdBy,
      },
