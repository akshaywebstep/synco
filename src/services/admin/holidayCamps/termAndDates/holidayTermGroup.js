...
const { HolidayTermGroup } = require("../../../../models");

// ✅ CREATE
exports.createHolidayGroup = async ({ name, createdBy }) => {
  try {
    const group = await HolidayTermGroup.create({ name, createdBy });
...
// ✅ GET ALL - by admin
exports.getAllHolidayGroups = async (adminId) => {
  try {
    ...
    const groups = await HolidayTermGroup.findAll({
...
exports.getHolidayGroupById = async (id, adminId) => {
  try {
    ...
    const group = await HolidayTermGroup.findOne({
...
exports.updateHolidayGroup = async (id, { name }, adminId) => {
  try {
    const group = await HolidayTermGroup.findOne({
...
exports.deleteHolidayGroup = async (id, deletedBy) => {
  try {
    // ✅ Find the HolidayTermGroup by ID and createdBy
    const group = await HolidayTermGroup.findOne({
