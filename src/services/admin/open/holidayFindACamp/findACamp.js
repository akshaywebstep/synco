...
exports.getAllHolidayVenuesWithHolidayClasses = async ({
    ...
    try {
        ...
        if (hasCoordinates) {
            console.log("✅ User coordinates provided:", {
                userLatitude,
                userLongitude,
                searchRadiusMiles,
            });
        ...
        if (hasCoordinates) {
            ...
            venues = await HolidayVenue.findAll({
                where: {
                    ...venueWhere,        // ✅ APPLY FILTERS
                    ...whereCondition,    // distance filter
                },
                attributes: {
                ...
                include: [
                    {
                        ...
                        required: true, // only venues WITH classes
                    },
                ],
        ...
        const formattedVenues = await Promise.all(
            venues.map(async (venue) => {
                if (
                    !venue.holidayClassSchedules ||
                    venue.holidayClassSchedules.length === 0
                ) {
                    return null;
                }
                ...
                return {
                    ...
                    paymentGroups: paymentGroups.map((pg) => ({
                        ...
                        holidayPaymentPlans: (pg.holidayPaymentPlans || []).map(
                            (plan) => ({
                                id: plan.id,
                                title: plan.title,
                                price: plan.price,
                                ...
                                PaymentGroupHasPlan:
                                    plan.PaymentGroupHasPlan || null,
                            })
                        ),
                    })),

                    holidayCamps: holidayCamps.map((camp) => ({
                        id: camp.id,
                        name: camp.name,
                    })),
        ...
        return {
            status: true,
            data: formattedVenues.filter(Boolean),
        };
    } catch (error) {
        console.error("❌ getAllHolidayVenuesWithHolidayClasses Error:", error);
        return {
            status: false,
