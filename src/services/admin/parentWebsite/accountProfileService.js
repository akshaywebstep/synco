...
const getBookingDate = (b) => {
    if (b.serviceType === "one to one") return b.date;
    if (b.serviceType === "birthday party") return b.partyDate;

    if (b.serviceType === "holiday camp")
        return b.classSchedule?.startDate || null;

    // weekly
    return (
        b.students?.[0]?.classSchedule?.startDate ||
        b.students?.[0]?.classSchedule?.date ||
        null
    );
};
...
exports.getMyBookingsByParentAdminId = async (parentAdminId) => {
    try {
        // Run both queries in parallel
        const [bookings, oneToOneLead, birthdayPartyLead, holidayBookings] = await Promise.all([
            Booking.findAll({
        ...
        combinedBookings.forEach((b) => {
            ...
            const bookingDateRaw = getBookingDate(b);
            if (!bookingDateRaw) {
                upcomingBookings.push(b); // fallback
                return;
            }

            const bookingDate = new Date(bookingDateRaw);

            if (bookingDate < now) pastBookings.push(b);
            else upcomingBookings.push(b);
        });
        return {
            status: true,
            message: "Fetched combined bookings successfully.",
