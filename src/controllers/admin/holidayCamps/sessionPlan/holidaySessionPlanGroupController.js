...
const SessionPlanGroupService = require("../../../../services/admin/holidayCamps/sessionPlan/holidaySessionPlanGroup");
...
const { HolidaySessionExercise, HolidaySessionPlanGroup } = require("../../../../models");
...
const PANEL = "admin";
const MODULE = "holiday-session-plan-group";
...
exports.duplicateHolidaySessionPlanGroup = async (req, res) => {
  try {
    ...
    const result = await SessionPlanGroupService.duplicateHolidaySessionPlanGroup(id, createdBy);
    ...
    await SessionPlanGroupService.updateHolidaySessionPlanGroup(
      newGroupId,
      { banner, ...uploadFields },
      createdBy
    );
    ...
    console.error("Error in duplicateHolidaySessionPlanGroup:", error);
    return res.status(500).json({ status: false, message: "Server error." });
  }
};

exports.createHolidaySessionPlanGroup = async (req, res) => {
  try {
    ...
    const result = await SessionPlanGroupService.createHolidaySessionPlanGroup(payloadWithoutFiles);
    ...
    const allGroups = await HolidaySessionPlanGroup.findAll({
      where: { createdBy },
      order: [["sortOrder", "ASC"]],
      attributes: ["id"],
    });
    ...
    await SessionPlanGroupService.reorderHolidaySessionPlanGroups(orderedIds, createdBy);
    ...
    await SessionPlanGroupService.updateHolidaySessionPlanGroup(sessionPlanId, updatePayload, createdBy);
    ...
    console.error("Server error in createHolidaySessionPlanGroup:", error);
...
exports.getAllHolidaySessionPlanGroups = async (req, res) => {
  try {
    const createdBy = req.admin?.id || req.user?.id;
    ...
    const result = await SessionPlanGroupService.getAllHolidaySessionPlanGroups({ createdBy });
    ...
    const sessionExercises = await HolidaySessionExercise.findAll({ where: { createdBy } });
...
exports.getHolidaySessionPlanGroupDetails = async (req, res) => {
  try {
    const { id } = req.params;
    const createdBy = req.admin?.id || req.user?.id;
    ...
    const result = await SessionPlanGroupService.getHolidaySessionPlanGroupById(id, superAdminId);
    ...
    if (DEBUG) console.error("Error in getHolidaySessionPlanGroupDetails:", error);
    return res.status(500).json({ status: false, message: "Server error." });
  }
};

exports.downloadHolidaySessionPlanGroupVideo = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const group = await HolidaySessionPlanGroup.findOne({
...
exports.updateHolidaySessionPlanGroup = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const existingResult = await SessionPlanGroupService.getHolidaySessionPlanGroupById(id, adminId);
    ...
    // STEP 8: Update DB
    const updateResult = await SessionPlanGroupService.updateHolidaySessionPlanGroup(id, updatePayload, adminId);
    ...
    await logActivity(
      req,
      "Admin", // PANEL name
      "Holiday-Session-Plan-Group", // MODULE name
      "update", // Action type
      {
...
exports.deleteHolidaySessionPlanGroup = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const deleteResult = await SessionPlanGroupService.deleteHolidaySessionPlanGroup(
      id,
      adminId
    );
...
exports.deleteHolidaySessionPlanGroupLevel = async (req, res) => {
  const { id, levelKey } = req.params;
  const adminId = req.admin?.id; // ✅

  console.log("============================================");
  console.log("📌 CONTROLLER: deleteHolidaySessionPlanGroupLevel");
  console.log("📌 Incoming Params:", { id, levelKey });
  console.log("➡️ Calling service.deleteLevelFromHolidaySessionPlanGroup...");

  try {
    const result =
      await SessionPlanGroupService.deleteLevelFromHolidaySessionPlanGroup(
        id,
        levelKey,
    ...
    const group = await HolidaySessionPlanGroup.findByPk(id, {
...
exports.reorderHolidaySessionPlanGroups = async (req, res) => {
  ...
  try {
    const result = await SessionPlanGroupService.reorderHolidaySessionPlanGroups(
      orderedIds,
      adminId
    );
    ...
    console.error("❌ reorderHolidaySessionPlanGroups controller error:", error);
    return res.status(500).json({
      status: false,
