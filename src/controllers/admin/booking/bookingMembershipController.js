...
exports.createBooking = async (req, res) => {
  ...
  try {
    ...
    // ✅ Validate parents (add this block below)
    ...
    for (const parent of formData.parents) {
      if (!parent.parentFirstName || parent.parentFirstName.trim() === "") {
        return res.status(400).json({
          status: false,
      ...
      if (!parent.parentLastName || parent.parentLastName.trim() === "") {
        return res.status(400).json({
          status: false,
      ...
      if (!parent.parentEmail || parent.parentEmail.trim() === "") {
        return res.status(400).json({
          status: false,
      ...
      if (!parent.parentPhoneNumber || parent.parentPhoneNumber.trim() === "") {
        return res.status(400).json({
          status: false,
      ...
      if (!phoneRegex.test(parent.parentPhoneNumber.trim())) {
        return res.status(400).json({
          status: false,
          message: `Parent phone number "${parent.parentPhoneNumber}" is not valid. Only digits, '+', '-', and spaces are allowed.`,
        });
      }
