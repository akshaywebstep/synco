...
const CandidateProfileService = require("../../../../services/admin/recruitment/franchise/franchiseCandidateProfile");
const FranchiseRecruitmentLeadService = require("../../../../services/admin/recruitment/franchise/franchiseRecruitmentLead");
...
exports.createFranchiseCandidateProfile = async (req, res) => {
    ...
    try {
        ...
        // ------------------------------
        //  SEND EMAIL TO RECRUITMENT LEAD
        // ------------------------------
        if (qualifyLead === true && recruitmentLeadId) {
            try {
                // 1️⃣ Fetch recruitment lead info
                const recruitmentLeadResult = await FranchiseRecruitmentLeadService.getFranchiseRecruitmentLeadById(recruitmentLeadId, adminId);

                if (!recruitmentLeadResult.status || !recruitmentLeadResult.data.email) {
                    console.warn(`⚠️ No email found for recruitmentLeadId=${recruitmentLeadId}`, recruitmentLeadResult.data);
                } else {
                    const leadEmail = recruitmentLeadResult.data.email;
                    const leadName = recruitmentLeadResult.data.name || "Recruitment Lead";
                    console.log(`📧 Found email: ${leadEmail}`);

                    // 2️⃣ Fetch email template
                    const { status: configStatus, emailConfig, htmlTemplate, subject } =
                        await emailModel.getEmailConfig(PANEL, "candidate-profile-franchise");
                ...
                console.error("❌ Error sending email to recruitment lead:", emailErr.message);
            }
        }
